...
The problem selector has one `Linear program` option. The Dimension / view selector switches it between the original 2D LP and a genuine 3D LP with `z in R^3`, `y in R^3`, a fixed tetrahedral decision polytope `Z = conv{v1,v2,v3,v4}`, generated 3D outcome samples, and a voxel-slice approximation of the true inverse epsilon-near-optimal region. Linear 2D keeps direct point dragging for the selected feasible decision `z` and the editable boundary vertices. Linear 3D instead uses barycentric decision weights `lambda1, ..., lambda4`, where the selected decision is `z = lambda1 v1 + lambda2 v2 + lambda3 v3 + lambda4 v4`. The weights are kept nonnegative and normalized to sum to one, so every selected 3D decision remains inside the tetrahedron. Clicking a 3D tetrahedron vertex selects that exact vertex by setting the corresponding one-hot weight vector. Linear 3D risk is restricted to Monte Carlo mode because p-value and e-value radii are only implemented for the 2D inverse-region geometry in this prototype.
...
Interaction in 3D mode is dependency-free canvas interaction: adjust Linear 3D's barycentric weight sliders or click a tetrahedron vertex to select a one-hot vertex decision, drag the outcome canvas to rotate the view, or focus the outcome canvas and use the arrow keys. `ArrowLeft`/`ArrowRight` rotate horizontally, `ArrowUp`/`ArrowDown` adjust pitch, and `Home` resets the 3D camera. Keyboard focus is enabled only while an active 3D visualization is shown. Tabs preserve their own selected dimension/view, 3D camera angle, problem class, Linear 2D decision, Linear 3D decision weights, Q configuration, and generated outcome settings.
